...
- Для разработки необходим dotnet sdk 10
- Node.js - рекомендуемая версия не ниже v22.14.0

1. **Установите переменные окружения:**
   - `ConnectionStrings__sqlite` - Если используется БД SQLite
   - `ConnectionStrings__StickedWordsDbContext` - строка подключения к базе данных, если используется Postgres
   - `DbProvider` - Название провайдера базы данных. Поддерживаются значения `SQLite` и `PostgreSQL`

2. **Сборка frontend:**
   - Перейдите в директорию `src/StickedWords.UI.React`
   - Установите пакеты: `npm i`
   - Запустите сборку проекта с пересборкой при изменении файлов: `npm run build`
