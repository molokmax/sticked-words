...
import { ChooseFromForeignExerciseService } from '../../../services/exercises/ChooseFromForeignExerciseService';
import { useEnterKey } from '../../../services/hooks';
import { useErrorListContext } from '../../ErrorList';
import Image from '../../Image';

import './ChooseFromForeignExercise.scss';
...
function ChooseFromForeignExercise({ flashCardId, onNext }: Props) {
    ...
    const service = useMemo(() => new ChooseFromForeignExerciseService(), []);
    ...
    if (isGuessChecked && isGuessCorrect) {
        resultsElement = (
            <div className="choose-from-foreign-exercise__correct-result">
                <div className="choose-from-foreign-exercise__result-label">{"\u2713"} Right!</div>
            </div>
        );
    } else if (isGuessChecked && !isGuessCorrect) {
        resultsElement = (
            <div className="choose-from-foreign-exercise__wrong-result">
                <div className="choose-from-foreign-exercise__result-label">{"\u2717"} Wrong</div>
            </div>
        );
    }
    ...
    const buttonsElement = loading || isGuessChecked
        ? (
            <button
                className="choose-from-foreign-exercise__check-button primary"
                disabled={ loading }
    ...
    return (
        <main className="choose-from-foreign-exercise">
            <div className="choose-from-foreign-exercise__exercise-description">
                Choose translation:
            </div>
            {
                imageId
                    ? (
                        <div className="choose-from-foreign-exercise__image-wrapper">
                            <Image
                                readonly={true}
                                imageId={imageId}
                            ></Image>
            ...
            <div className="choose-from-foreign-exercise__word">
                { word }
            </div>
            <div className="choose-from-foreign-exercise__options">
                {
                    options.map(x => (
                        <div
                            className={`choose-from-foreign-exercise__option ${getOptionClass(x)} ${isGuessChecked ? 'disabled'  : ''}`}
                            onClick={ () => onCheckClicked(x) }
                        >{x}</div>
                    ))
                }
            </div>
            <div className="choose-from-foreign-exercise__results">
                { resultsElement }
            </div>
            <div className="choose-from-foreign-exercise__buttons">
                { buttonsElement }
            </div>
        </main>
    );
...
export default ChooseFromForeignExercise;
